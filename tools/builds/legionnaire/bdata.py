...
PHASES = [
 dict(id="a1", name=L("Ato 1", "Act 1"), lv=[1, 15], tag=L("Cajado + Falling Thunder", "Quarterstaff + Falling Thunder"),
  ...
  tree=L("A região de crítico de cajado do build fica a 10 passos do início da Mercenary: o caminho é uma fila de nós de +5 Atributo até o primeiro Jewel Socket. Nada de notable ainda — os atributos já pagam os requisitos de Dex/Int das gems.", "The build's quarterstaff-crit region is 10 steps from the Mercenary start: the road is a line of +5 Attribute nodes up to the first Jewel Socket. No notable yet — the attributes already pay the Dex/Int requirements of your gems."),
 ...
 dict(id="a2", name=L("Ato 2", "Act 2"), lv=[16, 31], tag=L("Círculo de projéteis + primeiras cargas", "Projectile circle + first charges"),
  ...
  tree=L("Martial Artistry (+25 Dex, +25% de dano crítico com Quarterstaves) é a primeira notable do build e o ponto onde o caminho vira crítico de cajado.", "Martial Artistry (+25 Dex, +25% crit damage with Quarterstaves) is the build's first notable and the point where the road turns into quarterstaff crit."),
  ...
  exit=[L("1º Trial (~28): Essence of Virtue", "1st Trial (~28): Essence of Virtue"), L("Redflare Conduit e Powertread ficam prontos no nível 33", "Redflare Conduit and Powertread unlock at level 33"), L("Martial Artistry na árvore", "Martial Artistry on the tree")]),
 ...
 dict(id="a3", name=L("Ato 3", "Act 3"), lv=[32, 45], tag=L("Redflare Conduit: as cargas se geram sozinhas", "Redflare Conduit: charges make themselves"),
  ...
  tree=L("Agora que as cargas existem: The Fabled Stag (cargas duram +40% e podem não ser gastas), One with the Storm (o Falling Thunder consome uma carga a mais) e Stand and Deliver (projéteis a menos de 2 m).", "Now that charges exist: The Fabled Stag (charges last +40% and may not be spent), One with the Storm (Falling Thunder consumes one extra charge) and Stand and Deliver (projectiles within 2 m)."),
 ...
 dict(id="a4", name=L("Ato 4 e Interlúdios", "Act 4 and Interludes"), lv=[46, 64], tag=L("Ascendência e o 2º buff de carga", "Ascendancy and the 2nd charge buff"),
  ...
  tree=L("The Power Within (+1 carga máxima e +20% de dano crítico ao ganhar carga), depois o pacote de crítico: For the Jugular, Deadly Force e Heartbreaking. Confira sempre se as gems que você quer usar cumprem os requisitos de atributo.", "The Power Within (+1 maximum charge and +20% crit damage after gaining a charge), then the crit package: For the Jugular, Deadly Force and Heartbreaking. Always check that the gems you want to use meet their attribute requirements."),
 ...
 dict(id="maps", name=L("Mapas 65+", "Maps 65+"), lv=[65, 78], tag=L("Motoric Implants: +2 níveis nas gems de Dexterity", "Motoric Implants: +2 levels on Dexterity gems"),
  ...
  tree=L("Moment of Truth, Tainted Strike, Coming Calamity, Catalysis (20% de dano elemental com ataques), Crashing Wave, Heartstopping, Struck Through, Maiming Strike e Acceleration. Os Jewel Sockets recebem joias de crítico e ataque.", "Moment of Truth, Tainted Strike, Coming Calamity, Catalysis (20% elemental damage with attacks), Crashing Wave, Heartstopping, Struck Through, Maiming Strike and Acceleration. The Jewel Sockets take crit and attack jewels."),
 ...
 dict(id="endgame", name=L("Endgame T15", "Endgame T15"), lv=[79, 90], tag=L("Amuleto com Eternal Rage e mais níveis de skill", "Amulet with Eternal Rage and more skill levels"),
  ...
  tree=L("Aqui a árvore passa a ser a do PoB: ela parte do ponto inicial do Monk graças à joia Split Personality (socket 21984), por isso não gasta os 9 nós de Atributo do caminho da Mercenary — o respec libera esses pontos. Entram Overflowing Power (+2 cargas), Critical Exploit, True Strike, Flow Like Water, Careful Assassin e Throatseeker. Sem a joia, continue pelo caminho da Mercenary (a rota acima).", "From here the tree is the PoB's: it starts at the Monk starting point thanks to the Split Personality jewel (socket 21984), so it doesn't spend the 9 Attribute nodes of the Mercenary road — the respec frees those points. Overflowing Power (+2 charges), Critical Exploit, True Strike, Flow Like Water, Careful Assassin and Throatseeker come in. Without the jewel, keep following the Mercenary road (the route above)."),
...
KEY_PASSIVES = [
 dict(node="Martial Artistry", type="Notable", text=L("+25 Dexterity, +25% Accuracy e +25% de dano crítico com Quarterstaves.", "+25 Dexterity, +25% Accuracy and +25% crit damage with Quarterstaves."), when=L("Ato 2 → sempre", "Act 2 → forever"), why=L("Abre o crítico de cajado e paga o requisito de Dex do Falling Thunder.", "Opens quarterstaff crit and pays the Falling Thunder Dex requirement.")),
 dict(node="Stand and Deliver", type="Notable", text=L("Projéteis: +40% de Critical Damage Bonus e +25% de dano contra inimigos a menos de 2 m.", "Projectiles: +40% Critical Damage Bonus and +25% damage against enemies within 2 m."), when=L("Ato 3 → sempre", "Act 3 → forever"), why=L("O Falling Thunder sai do impacto perto de você: quase sempre está a menos de 2 m.", "Falling Thunder fires from impact close to you: almost always within 2 m.")),
 dict(node="The Fabled Stag", type="Notable", text=L("+40% de duração das cargas, +10 Dex e 10% de chance de uma skill não gastar as cargas.", "+40% charge duration, +10 Dex and a 10% chance for skills not to spend charges."), when=L("Ato 3 → sempre", "Act 3 → forever"), why=L("Soma com o Perpetual Charge: as cargas duram e rendem mais.", "Stacks with Perpetual Charge: charges last longer and go further.")),
 dict(node="One with the Storm", type="Notable", text=L("Skills de Quarterstaff que consomem Power Charges contam como consumindo uma carga adicional.", "Quarterstaff skills that consume Power Charges count as consuming an additional Power Charge."), when=L("Ato 3 → sempre", "Act 3 → forever"), why=L("Cada carga rende um projétil a mais no Falling Thunder.", "Each charge yields one more Falling Thunder projectile.")),
 dict(node="The Power Within", type="Notable", text=L("+1 Power Charge máxima e +20% de dano crítico ao ganhar uma carga.", "+1 maximum Power Charge and +20% crit damage after gaining a charge."), when=L("Ato 4 → sempre", "Act 4 → forever"), why=L("Mais cargas = mais projéteis.", "More charges = more projectiles.")),
...
TREE_STAGES = [
 dict(lv="1–15", focus=L("Caminho de Atributos até a região de crítico", "Attribute road to the crit region"), dmg="Falling Thunder", **{"def": L("Vida", "Life")}, spirit="—", dont=L("Desviar do caminho", "Leaving the road")),
 dict(lv="16–31", focus=L("Martial Artistry: crítico de cajado", "Martial Artistry: quarterstaff crit"), dmg="Falling Thunder + Nova", **{"def": L("Vida e resistências", "Life and resistances")}, spirit=L("nenhum ainda", "none yet"), dont=L("Esquecer de guardar currency para o nível 33", "Forgetting to save currency for level 33")),
 dict(lv="32–64", focus=L("Cargas (Fabled Stag, One with the Storm, Power Within) e crítico (Deadly Force, Jugular)", "Charges (Fabled Stag, One with the Storm, Power Within) and crit (Deadly Force, Jugular)"), dmg="Falling Thunder + Charged Staff", **{"def": L("Vida e resistências com runas", "Life and resistances with runes")}, spirit="Herald · Wind Dancer", dont=L("Ligar buffs sem Spirit", "Turning buffs on without Spirit")),
 dict(lv="65–100", focus=L("Crítico, dano elemental, velocidade e +2 cargas (Overflowing Power)", "Crit, elemental damage, speed and +2 charges (Overflowing Power)"), dmg="Falling Thunder", **{"def": L("Virtuous Barrier + Wind Dancer", "Virtuous Barrier + Wind Dancer")}, spirit="Charge Regulation · Eternal Rage", dont=L("Perder o requisito de Dex", "Losing the Dex requirement")),
]
...
UI = dict(
 ...
 treeIntro=L("Árvore real do PoB do poe.ninja, cortada por fases. Atos 1–2: o caminho de Atributos até a região de crítico de cajado. Ato 3: cargas (Fabled Stag, One with the Storm). Ato 4: The Power Within e o pacote de crítico. Mapas: mais crítico e dano elemental. Endgame: a árvore exata do PoB, com a joia Split Personality.", "Real tree from the poe.ninja PoB, cut into phases. Acts 1–2: the Attribute road to the quarterstaff-crit region. Act 3: charges (Fabled Stag, One with the Storm). Act 4: The Power Within and the crit package. Maps: more crit and elemental damage. Endgame: the PoB's exact tree, with the Split Personality jewel."),
...
CHAR = dict(
 ...
 rules=[
  ...
  dict(when=dict(lvMin=36, notOwn=["stag"]), lvl="warn", t=L("The Fabled Stag não pego", "The Fabled Stag not taken"), d=L("Cargas duram mais e podem não ser gastas.", "Charges last longer and may not be spent."), tab="arvore"),
  dict(when=dict(lvMin=36, notOwn=["storm"]), lvl="warn", t=L("One with the Storm não pego", "One with the Storm not taken"), d=L("O Falling Thunder consome uma carga a mais: 1 projétil a mais por carga.", "Falling Thunder consumes an extra charge: one more projectile per charge."), tab="arvore"),
...
TREE_RULES = [
 dict(when=dict(lvMin=36, notOwn=["stag"]), t=L("Pegue The Fabled Stag: cargas duram mais.", "Take The Fabled Stag: charges last longer."), node="The Fabled Stag"),
 dict(when=dict(lvMin=36, notOwn=["storm"]), t=L("One with the Storm: 1 projétil a mais por carga.", "One with the Storm: 1 more projectile per charge."), node="One with the Storm"),
