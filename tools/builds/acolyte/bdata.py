...
PHASES = [
 ...
 dict(id="a2", name=L("Ato 2", "Act 2"), lv=[16, 29], tag=L("Toxic Growth + Waking Dream", "Toxic Growth + Waking Dream"),
  ...
  rotation=[L("Vine Arrow no boss (a planta também pode ser envenenada)", "Vine Arrow on the boss (the plant can be poisoned too)"), L("Toxic Growth EM CIMA do boss (uma vez só)", "Toxic Growth ON TOP of the boss (once only)"), L("Poisonburst Arrow MIRANDO NAS PÚSTULAS até elas explodirem", "Poisonburst Arrow AIMED AT THE PUSTULES until they explode"), L("Só então Toxic Growth de novo", "Only then Toxic Growth again")],
  gems=[
 ...
 dict(id="a3", name=L("Ato 3", "Act 3"), lv=[30, 44], tag=L("Plague Bearer + Choice of Power", "Plague Bearer + Choice of Power"),
  ...
  rotation=[L("Clear: Poisonburst até o Plague Bearer chegar a 100%", "Clear: Poisonburst until Plague Bearer hits 100%"), L("Ative o Plague Bearer", "Activate Plague Bearer"), L("Boss: Vine Arrow → Toxic Growth em cima dele → Poisonburst NAS pústulas → Plague Bearer → Toxic Growth", "Boss: Vine Arrow → Toxic Growth on top of it → Poisonburst AT the pustules → Plague Bearer → Toxic Growth")],
  gems=[
...
UNIQUES = [
 ...
 U("The Lethal Draw", L("Aljava", "Quiver"), L("Arma", "Weapon"), "a2", L("Attack speed, 5 de vida por inimigo acertado e chance de perfurar.", "Attack speed, 5 life per enemy hit and chance to pierce."), L("Sustain de vida no Ato 2–3 sem perder dano.", "Life sustain in Act 2–3 without losing damage."), L("Aljava com dano plano.", "Quiver with flat damage.")),
 ...
 U("Death's Harp", L("Arco", "Bow"), L("Arma", "Weapon"), "a3", L("Dano crítico, vida e mana por inimigo morto e +250–325% de Surpassing chance de disparar uma flecha adicional.", "Crit damage, life and mana per enemy killed and +250–325% Surpassing chance to fire an additional arrow."), L("Uma flecha a mais = mais nuvens de veneno por disparo. Compare com a Splinterheart e fique com o que limpar mais rápido.", "One extra arrow = more poison bursts per shot. Compare with Splinterheart and keep whichever clears faster."), "Splinterheart"),
...
GEAR = [
 dict(slot=L("Arco", "Bow"), cheap="Splinterheart · Death's Harp (16–28)", value="Slivertongue runeforged (39–40)", full=L("Obliterator Bow: físico, crítico, Onslaught ao matar", "Obliterator Bow: physical, crit, Onslaught on kill"), affix=L("% físico; físico plano; + projéteis; flecha adicional", "% physical; flat physical; + projectiles; additional arrow"), lvls=[dict(lv=1, n=L("Arco magic com físico", "Magic bow with physical")), dict(lv=16, n="Splinterheart"), dict(lv=28, n="Death's Harp"), dict(lv=39, n="Slivertongue"), dict(lv=40, n=L("Runeforge (Medved's Crest)", "Runeforge (Medved's Crest)")), dict(lv=78, n="Obliterator Bow")], note=BOW_RUNE),
...
TROUBLESHOOT = [
 (L("Boss difícil no leveling (clear está bom)", "Bosses are hard while leveling (clear is fine)"), L("É quase sempre a pústula sem veneno. Sem veneno guardado ela tem pavio de 15 s e explode fraca; com veneno, explode antes, com raio maior e até +300% de dano. Faça nesta ordem: 1) Concentrated Area na Toxic Growth (as 5 pústulas se sobrepõem no boss); 2) lance a Toxic Growth EM CIMA do boss, uma vez só; 3) fique acertando Poisonburst Arrow NAS PÚSTULAS (não no boss) até elas explodirem; 4) Despair antes de começar (todo o seu dano é caos); 5) Vine Arrow para abrir, porque a planta também aceita veneno. Antes do nível 35 (Plague Bearer) e dos notables de veneno do Ato 3, o boss é naturalmente lento — não é você jogando errado.", "It's almost always an uncharged pustule. With no stored poison it has a 15 s fuse and a weak blast; with poison it detonates sooner, with a larger radius and up to +300% damage. Do it in this order: 1) Concentrated Area on Toxic Growth (the 5 pustules overlap on the boss); 2) cast Toxic Growth ON TOP of the boss, once only; 3) keep hitting Poisonburst Arrow AT THE PUSTULES (not the boss) until they blow; 4) Despair before you start (all your damage is chaos); 5) Vine Arrow to open, since the plant also takes poison. Before level 35 (Plague Bearer) and the Act 3 poison notables, bosses are naturally slow — it isn't you playing badly.")),
...
MECH = dict(
 ...
 sections=[
  dict(type="cards", cards=[
   ...
   [L("3. Pústulas: bombas que VOCÊ carrega", "3. Pustules: bombs YOU charge"), L("Toxic Growth joga 5 pústulas (o máximo que existe ao mesmo tempo). Sozinhas, elas têm pavio de 15 SEGUNDOS e explodem fraco. Cada veneno que você guarda nelas faz a explosão vir mais cedo, com raio maior e até +300% de dano. Ou seja: depois de lançar, você tem que ENCHER as pústulas de veneno — mire o Poisonburst Arrow NAS pústulas, não no boss. Plague Bearer (100%) e os tornados do Archon carregam todas de uma vez.", "Toxic Growth throws 5 pustules (the maximum that can exist at once). On their own they have a 15 SECOND fuse and explode weakly. Every poison you store in them makes the blast come sooner, with a bigger radius and up to +300% damage. So after casting you must FILL the pustules with poison — aim Poisonburst Arrow AT the pustules, not at the boss. Plague Bearer (100%) and Archon's tornadoes charge them all at once.")],
