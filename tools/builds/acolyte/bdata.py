...
CASES = [
 (L("E a Gas Arrow? Vale para o boss?", "What about Gas Arrow? Is it good for bosses?"), L("Dados do jogo: a nuvem dura 4 s, cresce 20% por segundo e ENVENENA SEM PRECISAR ACERTAR (o jogo marca 'poisons without hit'). Isso é ótimo em dois pontos: veneno garantido no boss sem depender de mira ou crítico, e enchimento rápido do Plague Bearer — que é justamente quem estoura as pústulas. O problema: a explosão de fogo da nuvem só acontece com uma skill Detonator ou com um inimigo em Ignite, e esta build não tem NENHUMA fonte de fogo, então metade da skill fica morta; o dano base dela também é baixo (0,3 do dano da arma no nível 1) e ela gasta um slot de gem e mana. Veredito: não substitui o Poisonburst Arrow (que alimenta Herald of Blood, Herald of Plague e Bursting Plague com hits), mas é um botão extra defensável só para boss no leveling. O que eu não consigo confirmar pelos dados: se a nuvem também carrega as pústulas (elas se carregam com veneno aplicado nelas, e a nuvem envenena sem hit). Teste assim: lance a Toxic Growth no boss e use SÓ Gas Arrow; se as pústulas explodirem antes dos 15 s, a nuvem carrega.", "Game data: the cloud lasts 4 s, grows 20% per second and POISONS WITHOUT HITTING (the game flags it as 'poisons without hit'). That is great in two ways: guaranteed poison on the boss without relying on aim or crit, and it fills Plague Bearer fast — which is exactly what pops the pustules. The problem: the cloud's fiery explosion only happens with a Detonator skill or an Ignited enemy, and this build has NO fire source, so half the skill is dead; its base damage is also low (0.3 of weapon damage at level 1) and it costs a gem slot and mana. Verdict: it doesn't replace Poisonburst Arrow (which feeds Herald of Blood, Herald of Plague and Bursting Plague with hits), but it is a defensible extra boss-only button while leveling. What I can't confirm from data: whether the cloud also charges the pustules (they are charged by poison applied to them, and the cloud poisons without hit). Test it: cast Toxic Growth on the boss and use ONLY Gas Arrow; if the pustules blow before 15 s, the cloud charges them.")),
