...
(async()=>{
 ...
 try{
  ...
  for(const [width,height] of sizes){
   ...
   for(const file of ['silverfist','oracle','tactician','infernalist','acolyte','pathfinder','smith'].flatMap(b=>[b+'/index.html',b+'/en.html'])){
    ...
    if(width<1440){
     ...
     await page.locator('#sbFab').click();await page.locator('[data-bn-tab="gear"]').click();await page.locator('[data-bn-go="gear"]').last().click();assert.equal(await page.locator('#v-gear').isVisible(),true);assert.equal(await page.locator('#side').isVisible(),false);
  ...
  await page.goto(base+'/oracle/index.html');await page.locator('#sbFab').click();await page.setViewportSize({width:1500,height:900});await page.waitForFunction(()=>!buildNow.open);assert.equal(await page.locator('main').evaluate(el=>el.inert),false);
  await page.setViewportSize({width:390,height:844});await page.locator('#sbFab').click();await page.locator('[data-bn-tab="skills"]').click();await page.keyboard.press('Escape');await page.reload();await page.locator('#sbFab').click();assert.equal(await page.locator('#bn-tab-skills').getAttribute('aria-selected'),'true');
  ...
  await page.emulateMedia({reducedMotion:'no-preference'});await page.locator('#sbFab').click();
