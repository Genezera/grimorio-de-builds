...
(async () => {
  ...
  const pages = (process.env.PAGES || 'silverfist/index.html,silverfist/en.html,oracle/index.html,oracle/en.html,index.html,en.html,rites/index.html,rites/en.html').split(',');
  try {
    for (const [w, h] of SIZES) {
      ...
      for (const file of pages) {
        ...
        const isBuild = /silverfist|oracle/.test(file);
