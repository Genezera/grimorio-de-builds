...
(async () => {
  ...
  const pages = (process.env.PAGES || ['silverfist','oracle','tactician','infernalist','acolyte','pathfinder','smith','martial','shaman'].flatMap(b => [b + '/index.html', b + '/en.html']).concat(['index.html', 'en.html', 'rites/index.html', 'rites/en.html']).join(',')).split(',');
  try {
    for (const [w, h] of SIZES) {
      ...
      for (const file of pages) {
        ...
        const isBuild = /silverfist|oracle|tactician|infernalist|acolyte|pathfinder|smith|martial|shaman/.test(file);
