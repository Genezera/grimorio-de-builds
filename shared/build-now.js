...
function buildNowContent(){
  ...
  if(buildNow.tab==='skills'){
    ...
    return `<div class="bn-section-heading"><h3>${T('Sua barra de skills','Your skill bar')}</h3><p>${T('Skill primeiro, supports logo abaixo. O status usa as marcações de Meu personagem.','Skill first, supports below. Status comes from My character.')}</p></div><div class="bn-skills">${gems.map(g=>{const key=(CHAR.skillMap||{})[g.skill],tracked=!!key,has=tracked&&own(key);return `<article class="bn-skill"><button type="button" class="bn-item" data-bn-go="skills">${buildNowImage(gemImg(g.skill))}<span class="bn-item-copy"><small>${esc(g.role||T('Skill da fase','Phase skill'))}${tracked?` · ${has?T('TENHO','OWNED'):T('FALTA','MISSING')}`:''}</small><b>${esc(g.skill)}</b><span>${g.sp==='opt'?T('Só se sobrar Spirit','Only with spare Spirit'):g.sp==='free'?T('Sem reserva de Spirit','No Spirit reservation'):T('Ver como usar','See how to use')} ↗</span></span></button>${(g.sup||[]).length?`<ul class="bn-supports" aria-label="Supports">${g.sup.map(s=>`<li>${supImg(s)?`<img src="${esc(supImg(s))}" alt="" loading="lazy">`:''}<span>${esc(s)}</span></li>`).join('')}</ul>`:''}</article>`}).join('')}</div>${buildNowLink('skills',T('Abrir guia de skills','Open skill guide'))}`;
  }
  ...
  return `<div class="bn-focus"><span class="bn-eyebrow">${T('Seu foco nesta fase','Your focus this phase')}</span><h3>${esc(p.carry)}</h3><p>${T('As sugestões usam seu nível, fase e escolhas em Meu personagem.','Suggestions use your level, phase and choices in My character.')}</p>${buildNowLink('meu',T('Atualizar meu personagem','Update my character'))}</div>
    ...
    <div class="bn-shortcuts">${buildNowLink('arvore',T('Próximas passivas','Next passives'))}${Object.hasOwn(VIEWS,'mech')?buildNowLink('mech',D.ui.mechBtn):buildNowLink(oracle?'totem':'zoo',oracle?T('Totems e mana','Totems and mana'):T('Planejar Spirit','Plan Spirit'))}</div>`;
}
function renderSide(){
